...
def test_projects_analyze(tmp_path):
    ...
    if analysis_dir.exists():
        shutil.copytree(analysis_dir, backup_dir / "analysis_output", dirs_exist_ok=True)

    try:
        # Execute command
        result = runner.invoke(app, ["projects", "analyze", "--start-date=2025-01-01"], catch_exceptions=False)
        ...
        for filename in ["engineering_taxonomy.csv", "lead_time.html",
                        "team_composition.html", "weekly_trends.html"]:
        ...
        if (backup_dir / "analysis_output").exists():
            ...
            shutil.copytree(backup_dir / "analysis_output", analysis_dir, dirs_exist_ok=True)
